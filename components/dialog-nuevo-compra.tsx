...
interface MyComponentProps {
  traerDatos: () => void;
  onCompraAgregada: () => void;
  prodSel: any;
  prov: any;
}

export function DialgoNuevaCompra({
  traerDatos,
  prodSel,
  prov,
  onCompraAgregada,
    ...
    if (postData.moneda === "") {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          // Notificar a TablaComprasRealizadas que se ha agregado una nueva compra
          onCompraAgregada();
        } else if (response.status === 500) {
          showToast.error("Ocurrio un error al registrar la compra", {
            duration: 4000,
            position: "top-right",
            transition: "fadeIn",
            sound: true,
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button variant="outline">Nueva Compra</Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[1200px] ">
        <DialogHeader>
          <DialogTitle>
            <u>Nueva Compra</u>: {prodSel.producto}
          </DialogTitle>
          <DialogDescription className="mt-1">
            Ingresa los datos del nueva compra.
          </DialogDescription>
        </DialogHeader>
        <form
          onSubmit={(e) => {
